...
# Verifica se já sincronizou os tenants nesta sessão
if 'tenants_synced' not in st.session_state:
    st.session_state.tenants_synced = False

# Inicializa variáveis de controle
tenant_options = {}
tenants = []

# Primeiro, verifica se há dados no banco local
session = Session()
try:
    db_tenants = session.query(NetboxTenant).all()
    local_tenant_count = len(db_tenants)
finally:
    session.close()

# Só sincroniza se:
# 1. Ainda não sincronizou nesta sessão E
# 2. Não há dados no banco local OU
# 3. É a primeira execução (session state não existe)
if not st.session_state.tenants_synced and (local_tenant_count == 0 or 'initial_load' not in st.session_state):
    
    # Marca que está fazendo o carregamento inicial
    st.session_state.initial_load = True
    
    # Consulta de tenants (com paginação)
    with st.spinner("Carregando tenants do NetBox..."):
    ...
    if tenants:
        with st.spinner("Sincronizando tenants com o banco de dados local..."):
            try:
                ...
                st.session_state.tenants_synced = True
                st.success("Tenants sincronizados com sucesso!")
            except Exception as e:
                st.error(f"Erro ao sincronizar tenants: {str(e)}")

# Carrega os tenants do banco local (após sincronização ou se já existiam)
session = Session()
try:
    db_tenants = session.query(NetboxTenant).all()
...
# Adiciona informação sobre o status da sincronização
if st.session_state.tenants_synced:
    st.sidebar.success(f"✅ {len(tenant_options)} tenants carregados do cache local")
else:
    st.sidebar.info("🔄 Sincronizando tenants...")

col_tenant, col_button = st.sidebar.columns([3, 2])
with col_tenant:
...
with col_button:
    if st.button("Selecionar"):
        ...
        st.session_state.tenant_selected = True

# Botão para forçar nova sincronização (opcional)
if st.sidebar.button("🔄 Atualizar Tenants", help="Força uma nova sincronização dos tenants do NetBox"):
    st.session_state.tenants_synced = False
 ...
 #       st.write("Serviço selecionado:", service_val)
        ...
        # ---------------------------------------------------------------------
            ...
            with col_update_btn:
                st.write("")  # Espaçamento
                st.write("")  # Espaçamento
                update_prefixes = st.button("🔄", help="Atualizar prefixos do ASN", key="update_asn_prefixes")

            col_local, col_local_mask, col_remote, col_remote_mask = st.columns([0.15, 0.05, 0.15, 0.1])
            with col_local:
                st.subheader("Peer Local")
                peer_local_v4 = st.text_input("Endereço IPv4 Local", value="", help="Endereço IPv4 da interface local")
                peer_local_v6 = st.text_input("Endereço IPv6 Local", value="", help="Endereço IPv6 da interface local")
            ...
            with col_remote_mask:
                ...
                peer_remoto_mask_v4 = st.text_input("Mask", value="", help="Máscara IPv4 do peer remoto")
                peer_remoto_mask_v6 = st.text_input("Mask", value="", help="Máscara IPv6 do peer remoto")
            col_check_mhop, col_mhop_v4, col_mhop_v6 = st.columns([0.1,0.45,0.45]) # Marcacao de Multi-Hop
            with col_check_mhop:
                st.subheader("MultiHop", help="Marcacao de Multi-Hop")
                check_mhop = st.checkbox("MHop")
            mhop_v4 = ""
            mhop_v6 = ""
            if check_mhop:
                with col_local:
                    mhop_v4 = st.text_input("MHop IPv4", value="", help="MHop para autenticação BGP")
                with col_remote:
                    mhop_v6 = st.text_input("MHop IPv6", value="", help="MHop para autenticação BGP")
            ...
            # Lógica de coleta de prefixos ASN
            ...
            should_update_prefixes = (
                (asn_remoto and not st.session_state.asn_lookup_done) or  # Primeira vez
                (asn_remoto != st.session_state.previous_asn) or  # ASN mudou
                update_prefixes  # Botão de atualização foi clicado
            )
            
            if should_update_prefixes and asn_remoto:
                with st.spinner(f"Buscando informações do ASN Remoto {asn_remoto}..."):
                    ...
                    # Mostra mensagem de sucesso se foi atualização manual
                    if update_prefixes:
                        st.success(f"Prefixos atualizados para AS{asn_remoto}: {len(ipv4_prefixes)} IPv4, {len(ipv6_prefixes)} IPv6")
        ...
        # ---------------------------------------------------------------------
            ...
            if selected_device_a:
                ...
                loopback_a = get_device_primary_ip(selected_device_a)
                # Use the new BGP configuration methods
                asn_local, asn_remoto, update_prefixes = bgp_config.show_basic_info()
                peer_info = bgp_config.show_peer_info()
                check_md5, md5_v4, md5_v6 = bgp_config.show_md5_config()
